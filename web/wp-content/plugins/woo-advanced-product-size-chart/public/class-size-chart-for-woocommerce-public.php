...
class Size_Chart_For_Woocommerce_Public
{
    ...
    public function size_chart_custom_product_tab_callback( $tabs )
    {
        ...
        if ( 0 == $size_chart_id || !$size_chart_id ) {
            $size_chart_id = $this->size_chart_id_by_tag( $post->ID );
            // Check if product is belongs to attribute
            if ( 0 == $size_chart_id || !$size_chart_id ) {
                $size_chart_id = $this->size_chart_id_by_attributes( $post->ID );
            }
        }
    ...
    public function size_chart_custom_product_tab_content_callback()
    {
        ...
        if ( 0 == $chart_id || !$chart_id ) {
            $chart_id = $this->size_chart_id_by_tag( $post->ID );
            // Check if product is belongs to attribute
            if ( 0 == $chart_id || !$chart_id ) {
                $chart_id = $this->size_chart_id_by_attributes( $post->ID );
            }
        }
     ...
     * @since 1.0.0
     */
    public function size_chart_id_by_tag( $product_id )
    {
        $size_chart_id = 0;
     ...
     * @since 1.0.0
     */
    public function size_chart_id_by_attributes( $product_id )
    {
        $size_chart_id = 0;
