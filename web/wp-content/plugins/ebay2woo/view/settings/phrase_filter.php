         ...
         <div class="panel panel-primary mt20" id="phrase_list">
                <div class="panel-heading">
                    <h3 class="panel-title"><?php _ex('Phrase Filtering (case sensitive)', 'Setting title', 'e2w'); ?></h3>
                    <span class="pull-right">
                        <a class="disabled" style="display: none;"><?php _e('You have unsaved changes', 'e2w'); ?></a>
                        <a href="#" class="apply-phrase-rules btn"><?php _e('Apply Filter to your Shop', 'e2w'); ?></a></span>
                </div>
                
                
                <div class="panel-body">
                    <div class="panel panel-default" id="e2w-panel-info" style="display: none;">
                      <div class="panel-heading">Applying filter progress  <button type="button" class="close" data-target="#e2w-panel-info" data-dismiss="alert"> <span aria-hidden="true">&times;</span><span class="sr-only">Close</span>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group input-block no-margin">
                                      <input type="text" value="<?php echo $phrase->phrase; ?>" class="form-control small-input e2w_phrase" />
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group input-block no-margin">
                                      <input type="text" value="<?php echo $phrase->phrase_replace; ?>" class="form-control small-input e2w_phrase_replace" />
                                </div>   
                            </div>
                            <div class="col-md-4">
                ...
                <?php endforeach; ?> 
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group input-block no-margin">
                                      <input type="text" class="form-control small-input e2w_phrase" placeholder="some phrase or word" />
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group input-block no-margin">
                                      <input type="text" class="form-control small-input e2w_phrase_replace" placeholder="replacement or empty" />
                                </div>   
                            </div>
                            <div class="col-md-4">
        ...
        <div class="row pt20 border-top">
            <div class="col-sm-12">
                   <input class="btn btn-success" id="save-phrases"  type="submit" value="<?php _e('Save settings', 'e2w'); ?>"/>
            </div>
        </div>
        ...
        <div class="modal-overlay modal-apply-phrases">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title"><?php _e('Apply Filter to your Shop', 'e2w'); ?></h3>
                    <a class="modal-btn-close" href="#"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-cross"></use></svg></a>
                </div>
                <div class="modal-body">
                    <label><?php _e('Select the update type', 'e2w'); ?></label>
                    <div style="padding-bottom: 20px;">
                        <div class="type btn-group" role="group">
                            <button type="button" class="btn btn-default" value="products"><?php _ex('Products', 'Apply Phrases', 'e2w'); ?></button>
                            <button type="button" class="btn btn-default" value="reviews"><?php _ex('Reviews', 'Aplly Phrases', 'e2w'); ?></button>
                            <?php /*
                            <button type="button" class="btn btn-default" value="shippings"><?php _ex('Shipping methods', 'Apply Phrases', 'e2w'); ?></button>
                            */ ?>
                            <button type="button" class="btn btn-default" value="all_types"><?php _ex('All', 'Apply Phrases', 'e2w'); ?></button>
                        </div>
                    </div>
                    <div class="scope">
                        <label><?php _e('Select the update scope', 'e2w'); ?></label>
                        <div>
                            <div class="scope btn-group" role="group">
                                <button type="button" class="btn btn-default" value="shop"><?php _ex('Shop', 'Apply Phrases', 'e2w'); ?></button>
                                <button type="button" class="btn btn-default" value="import"><?php _ex('Import List', 'Apply Phrases', 'e2w'); ?></button>
                                <button type="button" class="btn btn-default" value="all"><?php _ex('Shop and Import List', 'Apply Phrases', 'e2w'); ?></button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button class="btn btn-default close-btn" type="button"><?php _e('Close', 'e2w'); ?></button>
                    <button class="btn btn-success apply-btn" type="button"><?php _e('Apply', 'e2w'); ?></button>
                </div>
            </div>
        </div>
...
<script>
    (function ($) {
        ...
        var set_progress_bar_value = function(c, v){
            jQuery('#e2w-panel-info '+c+' .progress-bar')
            .css('width', v+'%')
        ...
        var get_status_filter = function (show){
            ...
            var data = {action: 'e2w_get_status_apply_phrase_rules'};
               ...
               var json = $.parseJSON(response);
                    if (json.state === 'error') {
                        ...
                        if (typeof json.review_valuenow !== 'undefined') {
                                ...
                                jQuery('#e2w-panel-info').fadeIn(400);
      ...
      //  get_status_filter();
        ...
        $(".modal-apply-phrases .apply-btn").on("click", function () {
          ...
          //  get_status_filter(true);
            ...
            var data = {action: 'e2w_apply_phrase_rules', type:$(".modal-apply-phrases .btn-group.type").data().value, scope:$(".modal-apply-phrases .btn-group.scope").data().value};
            jQuery.post(ajaxurl, data).done(function (response) {
        ...
        function check_phrases(){
               ...
               $('#phrase_list > .panel-body .has-error').removeClass('has-error');
                ...
                $('#phrase_list > .panel-body .row:gt(0)').each(function () {
                    if (!$(this).is(":last-child") && $(this).find(".e2w_phrase").length > 0 && $.trim($(this).find(".e2w_phrase").val())  == ''  ){
                        $(this).find(".e2w_phrase").addClass('has-error');
                        empty_check = false;    
                    }
                });
        ...
        function set_last_phrase_row_enability(show) {
            ...
            if (show) {
                row.find('.e2w_phrase_replace').removeClass('opacity50');
                row.find('.e2w_phrase_replace').prop('disabled', false);
           
            } else {
                 row.find('.e2w_phrase_replace').removeClass('opacity50').addClass('opacity50');

                 row.find('.e2w_phrase_replace').prop('disabled', true);
        ...
        function add_phrase_row(this_row){
            var row = $(this_row).parents('.panel-body').children('.row:last-child'), 
                ...
                new_row.find('.e2w_phrase').val('');
                new_row.find('.e2w_phrase_replace').val('');
        ...
        $('#phrase_list > .panel-body').on('keyup', 'input[type="text"]', function () {
            ...
            keyup_timer = setTimeout(function () {
                ...
                if (check_phrases() && $.trim($(this_row).parents('.panel-body').find(".row:last-child .e2w_phrase").val())  != '') {
        ...
        $('#save-phrases').on('click', function(){
            ...
            var data = {'action': 'e2w_update_phrase_rules', 'phrases': []};
            
            $('#phrase_list > .panel-body .row').each(function () {
                if (!$(this).is(":last-child") && !$(this).is(":first-child")){
                    var rule = {'phrase': $(this).find('.e2w_phrase').val(),
                        'phrase_replace': $(this).find('.e2w_phrase_replace').val()
                    };
