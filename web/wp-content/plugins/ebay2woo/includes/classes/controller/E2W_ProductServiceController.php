...
/**
 * Description of E2W_ProductServiceController
 *
 * @author Andrey
 * 
 * @autoload: e2w_init
 */
if (!class_exists('E2W_ProductServiceController')) {

    class E2W_ProductServiceController {
        ...
        public function delete_post_action($post_id) {
            ...
            E2W_Utils::delete_post_images($post_id);

            //second, mark variation as skip
            if (get_post_type($post_id) == 'product_variation') {
                $variation = new WC_Product_Variation($post_id);
                $parent_id = $variation->get_parent_id();
                $e2w_skip_meta = get_post_meta($parent_id, "_e2w_skip_meta", true);
                if ($e2w_skip_meta) {
                    $external_variation_id = get_post_meta($post_id, "external_variation_id", true);
                    if (!in_array($external_variation_id, $e2w_skip_meta['skip_vars'])) {
                        $e2w_skip_meta['skip_vars'][] = $external_variation_id;
                    }
                    update_post_meta($parent_id, "_e2w_skip_meta", $e2w_skip_meta);
                }
            }
        }
        ...
        public function save_product_variation($variation_id, $i) {
            ...
            if ($e2w_skip_meta) {
                ...
                if (in_array($variation->get_status(), array('publish', false))) {
                    $e2w_skip_meta['skip_vars'] = array_diff($e2w_skip_meta['skip_vars'], array($external_variation_id));
                } else {
                    if (!in_array($external_variation_id, $e2w_skip_meta['skip_vars'])) {
                        $e2w_skip_meta['skip_vars'][] = $external_variation_id;
                    }
