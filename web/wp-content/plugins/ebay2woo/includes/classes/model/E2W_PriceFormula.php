...
/**
 * Description of E2W_PriceFormula
 *
 * @author Andrey
 */
class E2W_PriceFormula {
    ...
    public function load($id = false) {
        ...
        if ($load_id) {
            $formula_list = E2W_PriceFormula::load_formulas_list(false);
    ...
    public function save() {
        ...
        e2w_set_setting('formula_list', array_values($formula_list));
        return $this;
    }
    ...
    public static function apply_formula($product, $round = 2, $type = 'all') {
        $formula = E2W_PriceFormula::get_formula_by_product($product);

        $product_price = E2W_PriceFormula::normalize_product_price($product);
        
        if ($formula && $product_price) {
            $use_compared_price_markup = e2w_get_setting('use_compared_price_markup');

            $price_cents = e2w_get_setting('price_cents');
            $price_compared_cents = e2w_get_setting('price_compared_cents');
            ...
            if (isset($product['sku_products']['variations']) && $product['sku_products']['variations']) {
                foreach ($product['sku_products']['variations'] as &$var) {
                    $formula = E2W_PriceFormula::get_formula_by_product($var);
                    if ($formula) {
    ...
    public static function get_default_formula() {
        $formula = e2w_get_setting('default_formula');
        return new E2W_PriceFormula($formula && is_array($formula) ? $formula : array('value' => 1, 'sign' => '*', 'compared_value' => 1, 'compared_sign' => '*'));
    }

    public static function set_default_formula($formula) {
        e2w_set_setting('default_formula', get_object_vars($formula));
    }

    public static function get_default_formulas() {
        $f1 = new E2W_PriceFormula();
        $f1->id = 1;
        $f1->min_price = 0;
        $f1->max_price = 10;
        $f1->sign = "*";
    ...
    public static function get_formula_by_product($product) {
        ...
        $use_extended_price_markup = e2w_get_setting('use_extended_price_markup');
        if ($use_extended_price_markup) {
            
            $product_price = E2W_PriceFormula::normalize_product_price($product);
            if($product_price){
                $formula_list = E2W_PriceFormula::load_formulas_list();
                foreach ($formula_list as $formula) {
                    $check = true;
        ...
        return $res_formula ? $res_formula : E2W_PriceFormula::get_default_formula();
    }
}
