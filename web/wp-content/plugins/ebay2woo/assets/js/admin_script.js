...
function e2w_tmce_getContent(editor_id, textarea_id) {
...
function show_notification(text, danger) {
    ...
    if (jQuery(".e2w-content .alert").length === 0) {
        jQuery('<div class="alert" role="alert">').prependTo(jQuery(".e2w-content"));
    }

    var alert = jQuery(".e2w-content .alert");
...
function update_product_info(data, on_update_calback) {
    data.action = 'e2w_update_product_info';
...
function update_variation_info(data, on_update_calback) {
    ...
    if (data.variations.length > 0) {
        var post_data = { action: 'e2w_update_variation_info', id: data.id, variations: data.variations.splice(0, 100) }
...
function chech_products_view() {
    ...
    if (products_to_load.length > 0) {
        var data = { 'action': 'e2w_load_shipping_info', 'id': products_to_load };
        jQuery.post(ajaxurl, data).done(function (response) {
            var json = jQuery.parseJSON(response);
            if (json.state !== 'ok') {
                console.log(json);
...
(function ($, window, document, undefined) {
    $(function () {
        if (e2w_chrome_extension_loaded === false) {
            $("#chrome-notify").show();
        }
        ...
        $("#e2w-do-filter").click(function () {
            $(this).closest('form').submit();
            return false;
        });

        jQuery("#e2w-search-form").submit(function () {
        ...
        $("#e2w-sort-selector").change(function () {
            jQuery("#e2w-search-form #e2w_sort").val($(this).val());
            jQuery("#e2w-search-form").submit();
            return false;
        });

        $("#e2w-search-pagination li a").click(function () {
            jQuery("#e2w-search-form #cur_page").val($(this).attr('rel'));
            jQuery("#e2w-search-form").submit();
        ...
        $('#modal-country-select').on('change', function (evt) {
            ...
            if (tmp_data.country != $(this).val()) {
                ...
                $('.modal-shipping .shipping-method').html('<div class="e2w-load-container"><div class="e2w-load-speeding-wheel"></div></div>');

                var data = { 'action': 'e2w_load_shipping_info', 'id': tmp_data.product_id, 'country': tmp_data.country };
                jQuery.post(ajaxurl, data).done(function (response) {
                    var json = jQuery.parseJSON(response);
                    if (json.state !== 'ok') {
                        console.log(json);
        ...
        function e2w_js_add_to_import(products_to_import, state, befor_load_calback, on_load_calback) {
            if (products_to_import.length > 0) {
                ...
                jQuery.post(ajaxurl, data).done(function (response) {
                    ...
                    e2w_js_add_to_import(products_to_import, state, befor_load_calback, on_load_calback);
                }).fail(function (xhr, status, error) {
                    console.log(error);
                    state.import_error_cnt++;
        ...
        $('.e2w-content .import_all').click(function () {
            var this_btn = $(this);
            $('.modal-confirm').confirm_modal({
                ...
                yes: function () {
                    ...
                    $('.product-card:not(.product-card--added)').each(function () {
                        products_to_import.push({ 'action': 'e2w_add_to_import', 'id': $(this).attr('data-id') });
                    });
                    if (products_to_import.length > 0) {
                        ...
                        e2w_js_add_to_import(products_to_import, state, on_befor_import, on_import_load);
                        e2w_js_add_to_import(products_to_import, state, on_befor_import, on_import_load);
                    }
                }
            });
        ...
        $(".product-card").find(".product-card__actions .btn").on("click", function () {
            ...
            if ($(_this).closest(".product-card").hasClass('product-card--added')) {
                ...
                var data = { 'action': 'e2w_remove_from_import', 'id': $(_this).closest(".product-card").attr('data-id') };
                jQuery.post(ajaxurl, data).done(function (response) {
                    var json = jQuery.parseJSON(response);
                    if (json.state !== 'ok') {
                        console.log(json);
                ...
                products_to_import = [];
                products_to_import.push({ 'action': 'e2w_add_to_import', 'id': $(_this).closest(".product-card").attr('data-id') });
        ...
        $("#import-by-id-url-btn").on("click", function () {
            ...
            if (product_id) {
                ...
                products_to_import = [];
                products_to_import.push({ 'action': 'e2w_add_to_import', 'id': product_id });
        ...
        function update_bulk_actions() {
            if ($('.e2w-product-import-list .select :checkbox:checked').length > 0) {
                $("#e2w-import-actions .action-with-check select option").first().text('Bulk Actions (' + $('.e2w-product-import-list .select :checkbox:checked').length + ' selected)');
        ...
        $('#e2w-import-actions .check-all').change(function () {
            var checkboxes = $('.e2w-product-import-list .select :checkbox').not($(this));
            if ($(this).is(':checked')) {
        ...
        $('.e2w-product-import-list [rel="images"] .image').click(function () {
        ...
        $('.e2w-product-import-list [rel="images"] .image .icon-gallery-box').click(function () {
            $(this).toggleClass('selected');
            $('.e2w-product-import-list [rel="images"] .image .icon-gallery-box').not(this).removeClass('selected');
        ...
        $('.e2w-product-import-list .select :checkbox').change(function () {
            update_bulk_actions();
        });

        jQuery(".e2w-product-import-list").on("click", ".post_import", function () {
            var this_btn = this;
            var product = $(this).parents('.product');
            var products_to_import = [];
            var data = { 'action': 'e2w_push_product', 'id': $(product).attr('data-id') };
            products_to_import.push(data);

            $(this_btn).addClass('load');
            var on_load = function (id, response_state, response_message, state) {
                if (response_state !== 'error') {
                    $('.e2w-product-import-list .select :checkbox[value="' + id + '"]').parents('.product').parents('.row').remove();
                    if ($('.e2w-product-import-list').find('.product').length === 0) {
                        $('#e2w-import-content').hide();
                        $('#e2w-import-empty').show();
                    }
            ...
            var state = { num_to_import: products_to_import.length, import_cnt: 0, import_error_cnt: 0 };
            e2w_js_post_to_woocomerce(products_to_import, state, on_load);

            return false;
        });


        $('#e2w-import-actions .action-with-check select').change(function () {
            var ids = [];
            $('.e2w-product-import-list .select :checkbox:checked:not(:disabled)').each(function () {
                ids.push($(this).val());
            });
            ...
            if ($(this).val() === 'remove') {
                ...
                $('.modal-confirm').confirm_modal({
                    title: 'Remove ' + $('.e2w-product-import-list .select :checkbox:checked').length + ' products',
                    body: 'Are you sure you want to remove ' + $('.e2w-product-import-list .select :checkbox:checked').length + ' products from your import list?',
                    yes: function () {
                        $('#e2w-import-actions .action-with-check').addClass('load');
                        $("#e2w-import-actions .action-with-check select").prop('disabled', true);

                        var data = { 'action': 'e2w_delete_import_products', 'ids': ids };
                        jQuery.post(ajaxurl, data).done(function (response) {
                            var json = jQuery.parseJSON(response);
                            if (json.state !== 'ok') {
                                console.log(json);
                            }
                            $('#e2w-import-actions .action-with-check').removeClass('load');
                            $("#e2w-import-actions .action-with-check select").prop('disabled', false);
                            $('#e2w-import-actions .action-with-check select').val(0);
                            $.each(ids, function (i, item) {
                                $('.e2w-product-import-list .select :checkbox[value="' + item + '"]').closest('.product').parents('.row').remove();
                            });
                            update_bulk_actions();
                ...
                $('.modal-confirm').confirm_modal({
                    title: 'Push ' + $('.e2w-product-import-list .select :checkbox:checked').length + ' products',
                    body: 'Are you sure you want to push ' + $('.e2w-product-import-list .select :checkbox:checked').length + ' products to woocommerce?',
                    yes: function () {
                        $('#e2w-import-actions .action-with-check').addClass('load');
                        $("#e2w-import-actions .action-with-check select").prop('disabled', true);
                        ...
                        var products_to_import = [];
                        $.each(ids, function (i, item) {
                            products_to_import.push({ 'action': 'e2w_push_product', 'id': item });
                        });

                        $('.e2w-product-import-list .select :checkbox:checked:not(:disabled)').closest('.product').find('.post_import').addClass('load');
                        ...
                        var on_load = function (id, response_state, response_message, state) {
                            ...
                            if (response_state !== 'error') {
                                $('.e2w-product-import-list .select :checkbox[value="' + id + '"]').closest('.product').parents('.row').remove();
                            }
        ...
        $('#e2w-import-actions .link_category_all').click(function () {
            $(".set-category-dialog .categories").attr('data-link-type', 'all');
            $(".set-category-dialog").fadeIn(200);
            return false;
        });

        $('#e2w-import-actions .delete_all').click(function () {
        ...
        $('#e2w-import-actions .push_all').click(function () {
            var this_btn = $(this);
            $('.modal-confirm').confirm_modal({
                ...
                yes: function () {
                    $(this_btn).addClass('load');
                    $('.e2w-product-import-list .select :checkbox:not(:disabled)').closest('.product').find('.post_import').addClass('load');

                    var data = { 'action': 'e2w_get_all_products_to_import' };
                    jQuery.post(ajaxurl, data).done(function (response) {
                        var json = jQuery.parseJSON(response);
                        if (json.state !== 'ok') {
                            console.log(json);
                        ...
                        $.each(json.ids, function (i, id) {
                            products_to_import.push({ 'action': 'e2w_push_product', 'id': id });
                        });
                        ...
                        var on_load = function (id, response_state, response_message, state) {
                            ...
                            if (response_state !== 'error') {
                                ...
                                $('.e2w-product-import-list .select :checkbox[value="' + id + '"]').closest('.product').find('.post_import').removeClass('load');
                            }
        ...
        function e2w_js_post_to_woocomerce(products_to_import, state, on_load_calback) {
            if (products_to_import.length > 0) {
                var data = products_to_import.shift();
                ...
                if (typeof need_update_product !== typeof undefined && need_update_product !== false) {
                    ...
                    if (typeof tinyMCE !== 'undefined') { update_data['description'] = encodeURIComponent(e2w_tmce_getContent(data.id)); }
                    update_product_info(update_data, function () {
                        ...
                        jQuery.post(ajaxurl, data).done(function (response) {
                            ...
                            e2w_js_post_to_woocomerce(products_to_import, state, on_load_calback);
                        }).fail(function (xhr, status, error) {
                            console.log(error);
                            state.import_error_cnt++;
        ...
        $('.e2w-content .product input.title').change(function () {
            var product = $(this).parents('.product');
        ...
        $(".e2w-content .product .select2, .e2w-content .product .select2-tags").on("select2:select", function (e) {
            var product = $(this).parents('.product');
            $(product).attr('data-need-update-product', true);
            update_product_info({ id: $(product).attr('data-id'), title: $(product).find('.title').val(), tags: $(product).find('.tags').val(), type: $(product).find('.type').val(), status: $(product).find('.status').val(), categories: $(product).find('.categories').val() }, function () { $(product).removeAttr('data-need-update-product'); });
        });
        $(".e2w-content .product .select2, .e2w-content .product .select2-tags").on("select2:unselect", function (e) {
            var product = $(this).parents('.product');
            $(product).attr('data-need-update-product', true);
            update_product_info({ id: $(product).attr('data-id'), title: $(product).find('.title').val(), tags: $(product).find('.tags').val(), type: $(product).find('.type').val(), status: $(product).find('.status').val(), categories: $(product).find('.categories').val() }, function () { $(product).removeAttr('data-need-update-product'); });
        });

        $('.e2w-content .variants-table .var_data input[type="text"]').change(function () {
            var product = $(this).closest('.product');
        ...
        $('.e2w-content .price-edit-selector .dropdown-menu a').click(function () {
        ...
        $('.e2w-content .price-edit-selector .price-box-top .apply').click(function () {
        ...
        $('.e2w-content .price-edit-selector .price-box-top .close').click(function () {
            $(this).parents('.price-box-top').hide();
        });

        $('.e2w-content .variants-actions .disable-var-price-change').change(function () {
            var $product = $(this).closest('.product');
        ...
        $('.e2w-content .variants-actions .disable-var-quantity-change').change(function () {
            var $product = $(this).closest('.product');
        ...
        var e2w_external_images_page_size = 1000;

        function e2w_calc_external_images(on_load_calback) {
            var data = { 'action': 'e2w_calc_external_images', 'page_size': e2w_external_images_page_size };
            jQuery.post(ajaxurl, data).done(function (response) {
                var json = jQuery.parseJSON(response);

                var block_data = $('#e2w_load_external_image_block').data();
        ...
        var e2w_calc_external_images_calback = function (ids, block_data) {
            images_to_load = [];
            jQuery.each(ids, function (i, id) {
                images_to_load.push({ 'action': 'e2w_load_external_image', 'id': id });
            });

            var on_load = function (state) {
                $("#e2w_load_external_image_progress").html(sprintf(e2w_common_data.lang.process_loading_d_of_d_erros_d, block_data.ok + state.import_cnt, block_data.total, block_data.error + state.import_error_cnt));

                if ((state.import_cnt + state.import_error_cnt) === state.num_to_import) {
                    block_data.ok += state.import_cnt;
                    block_data.error += state.import_error_cnt;

                    $('#e2w_load_external_image_block').data(block_data);

                    if ((block_data.ok + block_data.error) < block_data.total) {
                        e2w_calc_external_images(e2w_calc_external_images_calback);
                    } else {
                        location.reload();
                    }
                }
            ...
            e2w_load_external_image(images_to_load, state, on_load);
        };

        function e2w_load_external_image(images_to_load, state, on_load) {
        ...
        $('#e2w_load_external_image_block .load-images').click(function () {
            $('#e2w_load_external_image_block .load-images').attr('disabled', 'disabled');
            var block_data = $('#e2w_load_external_image_block').data();
            $("#e2w_load_external_image_progress").html(sprintf(e2w_common_data.lang.process_loading_d_of_d_erros_d, 0, block_data.total, 0));
            e2w_calc_external_images(e2w_calc_external_images_calback);
        });
        ...
        $('.set-category-dialog .modal-footer .btn').click(function () {
            if ($(this).hasClass('no-btn')) {
                ...
                if (new_categories) {
                    if (link_type === 'all') {
                        var ids = 'all';
                        $('.e2w-product-import-list .product select.categories').each(function () {
                    ...
                    jQuery.post(ajaxurl, { 'action': 'e2w_link_to_category', 'categories': new_categories, 'ids': ids }).fail(function (xhr, status, error) { console.log(error); });
                }
                $(this).parents(".modal-overlay").fadeOut(200);
            }
            return false;
        ...
        // init load button
        $('#e2w_load_external_image_block').data({ total: 0, ok: 0, error: 0 });
        var data = { 'action': 'e2w_calc_external_images_count' };
        jQuery.post(ajaxurl, data).done(function (response) {
            var json = jQuery.parseJSON(response);

            $('#e2w_load_external_image_block').data({ total: json.total_images, ok: 0, error: 0 });
            if (json.total_images > 0) {
                $('#e2w_load_external_image_block .load-images').removeAttr('disabled');
                $('#e2w_load_external_image_block .load-images').val(sprintf(e2w_common_data.lang.load_button_text, json.total_images));
            } else {
                $("#e2w_load_external_image_progress").html(e2w_common_data.lang.all_images_loaded_text);
            }
        ...
        $('#e2w-import-filter form select[name="o"]').change(function () {
